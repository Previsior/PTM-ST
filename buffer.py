...
def main(args):
    if args.disabled_wandb == True:
        ...
        wandb.init(project='LoRS-Buffer', config=args, 
                   name=f"{args.dataset}_{args.image_encoder}_{args.text_encoder}_{args.loss_type}_{no_aug_suffix}")
    ...
    save_dir = os.path.join(save_dir, args.image_encoder+"_"+args.text_encoder, args.loss_type+no_aug_suffix, "normal")
    ...
    data = np.load(f'./npz/{args.dataset}_{args.text_encoder}_text_embed.npz')
    ...
    for it in range(0, args.num_experts):
        ...
        for e in trange(args.train_epochs):
            train_loss, train_acc = epoch(trainloader, teacher_net, teacher_optim, args)

            if (e+1) % args.eval_freq == 0:

                score_val_i2t, score_val_t2i = epoch_test(testloader, teacher_net, args.device, bert_test_embed, args.image_trainable)
