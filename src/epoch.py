...
def epoch(dataloader, net, optimizer, args):
    """
    ...
    for i, data in (enumerate(dataloader)):
        ...
        if require_similarity:
            loss, acc = net(image, caption, similarity)
        else:
            loss, acc = net(image, caption)
...
@torch.no_grad()
def epoch_test(dataloader, model, device, bert_test_embed, image_trainable):
    model.eval()
    ...
    for image, img_id in dataloader: 
        ...
        if not image_trainable:
            image_feat = model.image_projection(image_feat)
...
def evaluate_synset_PTM(it_eval, net, images_train, labels_train, lr_img, lr_txt, similarity_train, testloader, args, bert_test_embed, syn_data_list, mom=0.9, l2=0.0005):
    net = net.to(args.device)
    length = len(syn_data_list)
    image_syn_list = []
    text_syn_list = []
    sim_list = []
    syn_lr_img_list = []
    ...
    for syn_data in syn_data_list:
        image_syn_list.append(syn_data.image_syn.to(args.device))
        text_syn_list.append(syn_data.text_syn.to(args.device))
        sim_generator = syn_data.sim_generator
        sim_params = sim_generator.get_indexed_parameters()
        similarity = copy.deepcopy(sim_generator.generate_with_param(sim_params).detach())
        sim_list.append(similarity.to(args.device))
        syn_lr_img_list.append(syn_data.syn_lr_img)
        syn_lr_txt_list.append(syn_data.syn_lr_txt)
    ...
    for sub_num in range(length + 1):
        if sub_num < length:
            images = image_syn_list.pop(0)
            labels = text_syn_list.pop(0)
            similarity = sim_list.pop(0)
        else:
            images = images_train
            labels = labels_train
            similarity = similarity_train
        
        trainloader = SimilarityDataloader(images, labels, similarity, batch_size=args.batch_train)
        ...
        for ep in tqdm(range(Epoch+1), ncols=60):
            loss_train, acc_train = epoch(trainloader, net, optimizer, args)
            acc_train_list.append(acc_train)
            loss_train_list.append(loss_train)
            if ep in eval_epochs:
                with torch.no_grad():
                    score_val_i2t, score_val_t2i = epoch_test(testloader, net, args.device, bert_test_embed, args.image_trainable)
                    val_result = itm_eval(score_val_i2t, score_val_t2i, testloader.dataset.txt2img, testloader.dataset.img2txt)

                    print("[Eval_{it:02d}] Subset{sub} Ep{ep} | Image R@1={img_r1:.2f} R@5={img_r5:.2f} R@10={img_r10:.2f} | Text R@1={txt_r1:.2f} R@5={txt_r5:.2f} R@10={txt_r10:.2f} | Mean={r_mean:.2f}".format(
                        it=it_eval, ep=ep, sub=sub_num, **val_result
                    ))
