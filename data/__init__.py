...
from data.cc3m_dataset import cc3m_train, cc3m_caption_eval, cc3m_retrieval_eval
import numpy as np
    ...
    if args.dataset in ['flickr', 'coco', 'cc3m']:
        if args.dataset == 'flickr':
            ...
            for i in range(0, len(texts), batch_size):
                ...
                batch_embed = net.text_encoder(batch)
                all_embeds.append(batch_embed)
            bert_test_embed = torch.cat(all_embeds, dim=0)

        bert_test_embed_np = bert_test_embed.cpu().numpy()
        np.savez(f'./npz/{args.dataset}_{args.text_encoder}_text_embed.npz', bert_test_embed=bert_test_embed_np) 
    else:
...
def textprocess_train(args, texts):
    ...
    if args.dataset in ['flickr', 'coco', 'cc3m']:
        np.savez(f'./npz/{args.dataset}_{args.text_encoder}_train_text_embed.npz', bert_test_embed=bert_test_embed_np) 
    else:
        raise NotImplementedError
    return 
...
def create_dataset(args, min_scale=0.5):
    ...
    if args.dataset=='flickr':
        ...
        train_dataset = cc3m_train(transform_train, args.image_root, args.ann_root)
